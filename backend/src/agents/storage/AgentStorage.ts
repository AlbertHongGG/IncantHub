...
export class AgentStorage {
  // __dirname is backend/src/agents/storage
  // we want to put it in backend/data/agents.json
  private static readonly FILE_PATH = path.resolve(__dirname, '..', '..', '..', 'data', 'agents.json');

  static async loadAll(): Promise<AgentConfig[]> {
    try {
      ...
      if (err.code === 'ENOENT') {
        // 檔案不存在，寫入預設 Seed Data
        await this.saveAll(DEFAULT_AGENTS);
  ...
  static async saveAll(agents: AgentConfig[]): Promise<void> {
    await fs.mkdir(path.dirname(this.FILE_PATH), { recursive: true });
    await fs.writeFile(this.FILE_PATH, JSON.stringify(agents, null, 2), 'utf-8');
  }
}
